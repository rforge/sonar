...
// LB: Hier braucht man dieses extern "C", damit er es in R versteht, außerdem void statt int.


extern "C" {
	
	
void floodfunc(double *bild, int *nrow, int *ncol)
{

	const int iOriginalColor = 1;
	
	int iYDimension = *nrow;
	int iXDimension = *ncol;

	CFloodFill FloodFill(bild, iXDimension, iYDimension);
